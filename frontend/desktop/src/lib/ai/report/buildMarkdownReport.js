 ...
 *   taskId: string,
 *   outcome: string,
 *   rounds: { round: number, vision: string, analysis: string, stepsExecuted: string[], roundEndVisionAssert?: { passed: boolean, evidence: string } | null }[],
...
export function buildMarkdownReport(p) {
  ...
  md += `## 最终验收\n\n- 通过：${p.finalAssertion.passed ? '是' : '否'}\n- 依据：${p.finalAssertion.evidence || '—'}\n\n`;
  md += `## 执行轮次\n\n`;
